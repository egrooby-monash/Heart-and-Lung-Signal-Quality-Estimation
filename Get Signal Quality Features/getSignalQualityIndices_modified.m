...
function  SQI= getSignalQualityIndices_modified(audio_data, Fs)
...
[truncated_autocorrelation, untruncated_autocorrelation] = get_autocorrelation(audio_downsampled,springer_options.audio_Fs);
...
[seSQI_5] = get_entropy(truncated_autocorrelation, 2, 0.1);
...
% Get the SVD sqi:
[svdSQI] = get_SVD_score_modified(truncated_autocorrelation,springer_options.audio_Fs);

% get ccSQI:
[ccSQI] = get_ccSQI_modified(untruncated_autocorrelation, springer_options.audio_Fs);

% Get mSQI:
[mSQI, ~] = get_max_peak_in_autocorrelation_modified(untruncated_autocorrelation, springer_options.audio_Fs);

% get bSQI
[bSQI] = get_bSQI_modified(audio_data,Fs);

% Get the sdrSQI
audio_downsampled_2000 = resample(audio_data,2000,Fs);
[sdrSQI] = get_signal_to_noise_psd_score_modified(audio_downsampled_2000,2000);
